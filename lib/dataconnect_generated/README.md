...
```dart
String id = ...;
ExampleConnector.instance.getUserRole(
  id: id,
).execute();
```
...
#### Return Type
`execute()` returns a `QueryResult<GetUserRoleData, GetUserRoleVariables>`
```dart
/// Result of an Operation Request (query/mutation).
class OperationResult<Data, Variables> {
  OperationResult(this.dataConnect, this.data, this.ref);
...
final result = await ExampleConnector.instance.getUserRole(
  id: id,
);
GetUserRoleData data = result.data;
final ref = result.ref;
```
...
final ref = ExampleConnector.instance.getUserRole(
  id: id,
).ref();
ref.execute();
